...
//    private var isTrackingLocation = false
    ...
    private var hasSentSOS: Boolean = false
    ...
    companion object {
        ...
        const val ACTION_REQUEST_SOS = "ACTION_REQUEST_SOS"
    ...
    private fun startLocationUpdates(uId: String = "none") {
...
//                val r2 = pref.getR2().toDoubleOrNull() ?: 0.0
                ...
                if (standbymain.curLat != 0.0 && standbymain.curLong != 0.0) {
                    ...
                    // 🔥 อัปเดตสถานะตามระยะทาง 🔥
//                    if (standbymain.distance <= r1) {
//                        standbymain.status = 0
//                    } else if (standbymain.distance > r1 && standbymain.distance <= r2) {
//                        standbymain.status = 1
//                    } else if (standbymain.distance > r2) {
//                        standbymain.status = 2
//                    }
//                    Log.d("GPS_STATUS", "Distance: ${standbymain.distance}, Status updated to: ${standbymain.status}")
                }

                Log.d("GPS", "Update lat=${loc.latitude}, lon=${loc.longitude}")

                if (!isServerAllowTrackingGps && hasSentSOS) {
                    Thread {
                        requestSOS(uId)
                    }.start()    // เรียก API ที่คุณมีอยู่แล้ว
                }
...
//                                // 🔥 [ส่วนที่เพิ่ม] เช็คว่ามีการอัปเดต Safe Zone หรือไม่
//                                if (json.has("update_safezone")) {
//                                    try {
//                                        // เช็คก่อนว่าค่าที่ส่งมาเป็น true หรือไม่ (เผื่อส่งมาเป็น false)
//                                        val isUpdate = json.getBoolean("update_safezone")
//
//                                        if (isUpdate) {
//                                            // ดึงค่าจาก JSON (ต้องตกลงชื่อ Key กับ Backend ให้ตรงกันเป๊ะๆ นะครับ)
//                                            val newLatStr = json.getString("safe_zone_lat")
//                                            val newLongStr = json.getString("safe_zone_long")
//                                            val newR1Str = json.getString("radius1")
//                                            val newR2Str = json.getString("radius2")
//
//                                            // 1. บันทึกลง Persistent Storage (Mem เครื่อง)
//                                            preferenceData.setLat(newLatStr)
//                                            preferenceData.setLong(newLongStr)
//                                            preferenceData.setR1(newR1Str)
//                                            preferenceData.setR2(newR2Str)
//
//                                            // 2. อัปเดตตัวแปร Global (เพื่อให้ Logic คำนวณระยะทางใช้ค่าใหม่ทันที)
//                                            // ใช้ toDoubleOrNull() กันแอปเด้งถ้าค่ามาผิด
//                                            standbymain.safeZoneLat = newLatStr.toDoubleOrNull() ?: standbymain.safeZoneLat
//                                            standbymain.safeZoneLong = newLongStr.toDoubleOrNull() ?: standbymain.safeZoneLong
//                                            standbymain.r1 = newR1Str.toDoubleOrNull() ?: standbymain.r1
//                                            standbymain.r2 = newR2Str.toDoubleOrNull() ?: standbymain.r2
//
//                                            Log.d("GPS_UPDATE", "✅ อัปเดตพิกัด Safe Zone ใหม่จาก Server เรียบร้อย!")
//                                            Log.d("GPS_UPDATE", "New Lat: $newLatStr, New R1: $newR1Str")
//                                        }
//                                    } catch (e: Exception) {
//                                        Log.e("GPS_UPDATE", "❌ Error parsing safezone data: ${e.message}")
//                                    }
//                                }
